...
def main():
    ...
    # scratch file to try to group by date more efficiently

    with open(args.input) as f:
        products = json.load(f)

    ps = list(zip(products.keys(), products.values()))
    print(len(ps))
    ys = (seq(ps)
        .group_by(lambda p: (p[1]['attrs']['month'], p[1]['attrs']['day']))
        .map(lambda g: g[0]) # grouping key
    ).list()
    print(ys)
    ...
    #     # add_by_date(products_by_date, name, attrs, files)
        ...
        # products_by_date_path = os.path.join('.', args.outdir, 'products_by_date.json')
        # with open(products_by_date_path), 'w') as f:
