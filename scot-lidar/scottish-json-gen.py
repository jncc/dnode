...
def get_products(grids, s3_path, bucket, s3_region, s3_bucket, base_title, type):
    ...
    for key in bucket.list(prefix=s3_path):
        grid = None
        print(key.key)
        if (type is 'DSM' and key.key.endswith('.tif')):
            grid = os.path.basename(key.key).replace('.tif', '').replace('DSM_', '')
        elif (type is 'DTM' and key.key.endswith('.tif')):
            grid = os.path.basename(key.key).replace('.tif', '').replace('DTM_', '')
        elif (type is 'LAS' and key.key.endswith('.las')):
            grid = os.path.basename(key.key).replace('.las', '').replace('LAS_', '')
        if (grid is not None):
            products.append({
                "id": str(uuid.uuid4()),
